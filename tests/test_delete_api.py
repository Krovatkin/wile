#!/usr/bin/env python3
"""
Test delete using the proper file operations flow
"""
...
print("=" * 70)
print("DELETE TEST WITH FILE OPERATIONS")
print("=" * 70)
...
try:
    with sync_playwright() as p:
        ...
        # Select file1.bin by clicking on it
        file1 = page.locator('li:has(span:text-is("file1.bin"))').first
        file1.click()
        time.sleep(1)
        print("3. Selected file1.bin")

        # Check if selection count updated
        selection_count = page.locator('#selectionCount').text_content()
        print(f"   Selection count: {selection_count}")

        # Use keyboard shortcut or call delete via URL
        # Let's try calling the /manage endpoint directly
        file1_path = "folder_A/file1.bin"

        # Make a fetch request to delete
        result = page.evaluate(f"""
            fetch('/manage?srcs={file1_path}&action=delete')
                .then(r => r.json())
                .then(data => data)
        """)
        time.sleep(2)
        print(f"4. Delete API result: {result}")
        ...
        page.screenshot(path='test_delete_api.png', full_page=True)
        print("\n✓ Screenshot saved: test_delete_api.png")
